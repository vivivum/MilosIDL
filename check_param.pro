...
if n_comp eq 2 then begin
  ...
  ;Magnetic field
  if (P_M(1+11*(n_comp-1)) lt 0) and (PLIMITS(1+11*(n_comp-1)).SET EQ 1) then begin
	P_M(1+11*(n_comp-1)) = (-1d0)*P_M(1+11*(n_comp-1))
	P_M(5+11*(n_comp-1)) = 180d0 - P_M(5+11*(n_comp-1))  
;    ch(1)=1
  endif
  ;Inclination
  If PLIMITS(5+11*(n_comp-1)).SET EQ 1 THEN BEGIN
  If P_M(5+11*(n_comp-1)) LT PLIMITS[5+11*(n_comp-1)].LIMITS(0)  THEN begin
    P_M(5+11*(n_comp-1)) = -P_M(5+11*(n_comp-1))
;    ch(5)=1
  endif
  If P_M(5+11*(n_comp-1)) GT PLIMITS[5+11*(n_comp-1)].LIMITS(1)  THEN begin
    P_M(5+11*(n_comp-1)) = 360d0-P_M(5+11*(n_comp-1))
;    ch(5)=1
  endif
  ENDIF

  ;azimuth
  If PLIMITS(6+11*(n_comp-1)).SET EQ 1 THEN BEGIN
  If P_M(6+11*(n_comp-1)) LT 0. THEN  begin
    P_M(6+11*(n_comp-1)) = 180d0 + P_M(6+11*(n_comp-1))
;    ch(6)=1
  endif
  If P_M(6+11*(n_comp-1)) GT PLIMITS[6+11*(n_comp-1)].LIMITS(1) THEN  begin
    P_M(6+11*(n_comp-1)) = P_M(6+11*(n_comp-1)) - 180d0
;    ch(6)=1
  endif
  ENDIF

  for i=0,4 do IF PLIMITS(i+11*(n_comp-1)).SET EQ 1 THEN $
	P_M(i+11*(n_comp-1)) = PLIMITS(i+11*(n_comp-1)).LIMITS(0) > P_M(i+11*(n_comp-1)) < PLIMITS(i+11*(n_comp-1)).LIMITS(1)
  for i=7,10 do IF PLIMITS(i+11*(n_comp-1)).SET EQ 1 THEN $
	P_M(i+11*(n_comp-1)) = PLIMITS(i+11*(n_comp-1)).LIMITS(0) > P_M(i+11*(n_comp-1)) < PLIMITS(i+11*(n_comp-1)).LIMITS(1)
