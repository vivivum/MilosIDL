...
;   THIS ONE work in NLTE. Nov, 2019. DOS -> A. Dorantes PhD ( numerically + almost full analitically)
;-
...
IF (D_N EQ 0) OR (D_N GE 2) then begin  ;ANALITIC RESPONSE FUNCTIONS
...
ENDELSE
    ...
    IF keyword_set(NLTE) THEN BEGIN
        ...
        ;D_EI_NLTE(*,0) = D_EI(*,0)+ETAIN/E0*DE0 ;SAME because d(ap1*MU + 1 + ETAI)/d(e0) = d(ETAI)/d(e0)
...
dd=d_spectra
    FOR PAR=0,3 DO BEGIN  ;loop in Stokes profiles
        FOR ITER=0,6 DO BEGIN    ;loop in model PARAMeters
			...
			ENDIF
        ENDFOR
        ;Parameter 7 is missing (is a constant)
        	ITER = 8
            If abs(Mean(D_SPECTRA(*,ITER,PAR))) GT 1.e-25 then begin  ; Just in case...
            	FFTD=FFT(D_SPECTRA(0:NUML-edge,ITER,PAR),-1,/double)
	 			; The convolution of Response Functions
            ...
            IF keyword_set(NLTE) THEN BEGIN
                ...
                ;     ENDIF
                ; ENDFOR
                ;Ojo. Esto de arriba se usará cuando la parte analítica de NLTE de 11 y 13esté bien. 
                ;como ahora se hacen numericamente, solo tengo que hacerlo para 10 y 12 no para 11 y 13
                ;ojo. No está así par la presencia del filtro. Si lo hay, hay que revisarlo si aun está esto sin corregir
                IP = [10,12]
                FOR Ipc = 0,n_elements(IP)-1 do begin
                    ITER = IP[Ipc]
                    If abs(Mean(D_SPECTRA(*,ITER,PAR))) GT 1.e-25 then begin  ; Just in case...
...
If D_N GE 1 then begin  ;NUMERICAL RESPONSE FUNCTIONS
    ...
    If (D_N EQ 2) THEN BEGIN  ;ANALITIC RESPONSE FUNCTIONS
        ...
        pause
    ENDIF
     If (D_N EQ 3) THEN BEGIN  ;ANALITIC RESPONSE FUNCTIONS

        SPECTRA_N = YFIT
        D_SPECTRA_N = PDER

        window,1,xsize=1400,ysize=400
        if keyword_set(nlte) then !p.multi=[0,8,2] else !p.multi=[0,6,2] 
        FOR J=0,nterms*n_comp-1 DO BEGIN
            PLOT,D_SPECTRA(*,j,0)-D_SPECTRA_N(*,j,0),TITLE='I',charsize=3
        ENDFOR
        window,2,xsize=1400,ysize=400
        if keyword_set(nlte) then !p.multi=[0,8,2] else !p.multi=[0,6,2] 
        FOR J=0,nterms*n_comp-1 DO BEGIN
            PLOT,D_SPECTRA(*,j,1)-D_SPECTRA_N(*,j,1),TITLE='Q',charsize=3
        ENDFOR
        window,3,xsize=1400,ysize=400
        if keyword_set(nlte) then !p.multi=[0,8,2] else !p.multi=[0,6,2] 
        FOR J=0,nterms*n_comp-1 DO BEGIN
            PLOT,D_SPECTRA(*,j,2)-D_SPECTRA_N(*,j,2),TITLE='U',charsize=3
        ENDFOR
        window,4,xsize=1400,ysize=400
        if keyword_set(nlte) then !p.multi=[0,8,2] else !p.multi=[0,6,2] 
        FOR J=0,nterms*n_comp-1 DO BEGIN
            PLOT,D_SPECTRA(*,j,3)-D_SPECTRA_N(*,j,3),TITLE='V',charsize=3
        ENDFOR
        pause
    ENDIF
    
ENDIF 
IF (D_N EQ 1) THEN BEGIN
  SPECTRA = YFIT
  D_SPECTRA = PDER
ENDIF
