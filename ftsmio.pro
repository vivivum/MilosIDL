pro ftsmio,data,wl,nblocks,xlam=xlam,dir=tdir,plot=iplot,sdir=sdir
;+
; NAME:
;	FTSREAD
; PURPOSE:
...
;sdir=getenv('IDL_DIR')+'/data/'
if not(keyword_set(sdir)) then sdir='/Users/orozco/Dropbox\ \(IdAdA\)/IDL/Rutinas/data/'

if not keyword_set(tdir) then mycd,tdir
print,sdir,tdir
...
cmd='dd if='+sdir+'/fts_cent.dat of='+tdir+ $
    '/tmp bs=1000 skip='+strtrim(skip,1)+' conv=swab count='+strtrim(nblocks,1)
